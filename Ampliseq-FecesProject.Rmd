...
# Adonis test
set.seed(867)
adonis2(bray ~ Sample_type*Hive, data = sampledf) # Interaction not significant
adonis2(bray ~ Sample_type+Hive, data = sampledf)
...
# Add cage to metadata
sampledf$Cage <- paste0(sampledf$Hive, sampledf$Sample_type)

# test of homogeneity of dispersion
beta <- betadisper(bray, sampledf$Cage)
...
# Metadata by cage
idTest <- unique(sampledf[c("Cage","Sample_type")])
...
adonis2(cage_centroids ~ idTest$Sample_type) # main effect
...
library(lmerTest)
results <- lmer(value ~ Sample_type * Hive + (1|Pair), data = alpha1.sh) # Interaction significant
car:::Anova(results)

results <- lmer(value ~ Sample_type * Hive + (1|Pair), data = alpha1.invSim) # Interaction not significant
car:::Anova(results)
results <- lmer(value ~ Sample_type + Hive + (1|Pair), data = alpha1.invSim) 
car:::Anova(results)
...
library(lmerTest)
results <- lmer(value ~ Sample_type * Hive + (1|Pair), data = alpha2.sh) # Interaction not significant
car:::Anova(results)
results <- lmer(value ~ Sample_type + Hive + (1|Pair), data = alpha2.sh)
car:::Anova(results)

results <- lmer(value ~ Sample_type * Hive + (1|Pair), data = alpha2.invSim) # Interaction not significant
car:::Anova(results)
results <- lmer(value ~ Sample_type + Hive + (1|Pair), data = alpha2.invSim)
car:::Anova(results)
```
...
results <- lmer(value ~ Sample_type * Hive + (1|Pair), data = alpha1.invSim) # Interaction almost significant (p = 0.051)
car:::Anova(results)
```
